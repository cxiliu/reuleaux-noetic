  ...
  :group_name_(group_name), check_collision_(check_collision)
{
  ...
  // const moveit::core::RobotModelConstPtr& kinematic_model = group_->getRobotModel();
  // ROS_INFO("Model frame: %s", kinematic_model->getModelFrame().c_str());
  moveit::core::RobotStatePtr kinematic_state(new moveit::core::RobotState(group_->getRobotModel()));
  kinematic_state->setToDefaultValues(); // use default values as seed position
  const moveit::core::RobotState& state_ref = *kinematic_state;
  moveit::core::robotStateToRobotStateMsg(state_ref, initial_state_);
}
 ...
 geometry_msgs::PoseStamped ReachAbility::makePoseStamped(const geometry_msgs::Pose& pose_in)
{
 ...
 moveit_msgs::PositionIKRequest ReachAbility::makeServiceRequestWithState(const geometry_msgs::Pose &pose_in)
 {
   moveit_msgs::PositionIKRequest req;
   geometry_msgs::PoseStamped pose_st = makePoseStamped(pose_in);
   req.group_name = group_name_;
  ...
  //  req.attempts = 10;
   req.timeout.fromSec(0.1);
   req.pose_stamped = pose_st;
   req.robot_state = initial_state_;
   return req;
 }
 ...
 bool ReachAbility::getIKSolution(const geometry_msgs::Pose &pose, moveit_msgs::RobotState &robot_state)
 {
  ...
  moveit_msgs::PositionIKRequest req = makeServiceRequestWithState(pose);
